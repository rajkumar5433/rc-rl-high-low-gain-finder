...
def main():
    ...
    for frq in frequencies:
        gain = find_gain(filter_type, frq, resistance, C_or_L)
        gains.append(gain)
        data.append({
            "Frequency (Hz)": frq,
            "Input voltage (V)": in_voltage,
            "Output voltage (V)": round(in_voltage * gain, OUTPUT_VOLTAGE_DECIMAL_PLACES),
            "Gain": round(gain, GAIN_DECIMAL_PLACES)
        })
    ...
    plt.plot(frequencies, gains, marker='o', linestyle='-', color='b')
    plt.xscale('log')  # Logarithmic x-axis is typical for frequency response
    plt.xlabel("Frequency (Hz)")
    plt.ylabel("Gain")
    plt.title(f"Frequency Response: {filter_type}")
